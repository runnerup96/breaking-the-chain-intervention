...
if __name__ == "__main__":
    ...
    parser.add_argument("--evaluation_dataset", type=str, required=True,
                        choices=["ricechem", "averitec", "pauq"])
    ...
    if args.evaluation_dataset == "ricechem":
        ...
        dataset = pauq_dataset.PAUQCorrectionDataset(path=dataset_path)
        dataloader = DataLoader(dataset, batch_size=args.batch_size, collate_fn=lambda b: b, shuffle=False)
        intervention_logic = pauq_intervention_2.PAUQIntervention(dataset, llm)
        evaluator = pauq_evaluation.PAUQCorrectionEvaluation(dataset)
    else:
    ...
    for batch in tqdm(dataloader, desc=f"Correction ({args.evaluation_dataset})", total=len(dataloader)):
        ...
        for s in batch:
            ...
            if args.evaluation_dataset == "pauq":
                tmp["schema_links"] = tmp["bad_schema_links"]
                tmp["skeleton"] = tmp["bad_skeleton"]
                tmp["slots"] = tmp["bad_slots"]
